...
process.env.KMOR_LOG_DIR = resolve(tmpdir(), "kmor-sidecar-test-logs");
...
describe("ClaudeSessionManager.sendMessage", () => {
	...
	test("emits contextUsageUpdated from the terminal result's usage + modelUsage, stamping the requested modelId", async () => {
		...
		expect(ctxUsage?.sessionId).toBe("kmor-sess-ctx");
	...
	test("getContextUsage slow path spawns a transient Query + returns rich meta", async () => {
		// No live session for this kmor id — slow path kicks in. The
	...
	test("ignores fastMode for models not in the hardcoded catalog", async () => {
		...
		await manager.sendMessage(
			"REQ-fast-sonnet",
			{
				sessionId: "kmor-sess-fast-sonnet",
				prompt: "test",
	...
	test.each([
		...
		await manager.sendMessage(
			`REQ-effort-${level}`,
			{
				sessionId: `kmor-sess-effort-${level}`,
				prompt: "test",
				model: "default",
				cwd: undefined,
				resume: undefined,
	...
	test("drops unknown effort levels instead of forwarding them", async () => {
		...
		await manager.sendMessage(
			"REQ-effort-bogus",
			{
				sessionId: "kmor-sess-effort-bogus",
				prompt: "test",
				model: "default",
				cwd: undefined,
				resume: undefined,
	...
	test("forwards only user-linked directories to Claude query options", async () => {
		const userDirA = makeTempDir("kmor-claude-user-a-");
		const userDirB = makeTempDir("kmor-claude-user-b-");
	...
	test("prepends the linked-directories preamble to Claude's first user message", async () => {
		const linkedDirA = makeTempDir("kmor-claude-prompt-a-");
		const linkedDirB = makeTempDir("kmor-claude-prompt-b-");
	...
	test("listSlashCommands forwards additionalDirectories and env", async () => {
		const workspaceDir = makeTempDir("kmor-claude-slash-");
		const linkedDir = makeTempDir("kmor-claude-slash-linked-");
		...
		expect(captured).toContainEqual({
			...
			message: {
				content: [
					{
						type: "text",
						text: '{"action":"accept","content":{"name":"Kmor"}}',
					},
				],
			},
		});
