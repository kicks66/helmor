 ...
 * the IME segments their input (e.g. `kmor` → candidate row shows
 ...
 * editor ends up with `he lmor` instead of the `kmor` the user actually
 * typed.
 *
 ...
 *   - WebKit (Kmor's Tauri target) sometimes does not fire `compositionend`
...
import { createKmorQueryClient } from "@/lib/query-client";
...
describe("WorkspaceComposer — IME switch mid-composition leaves segmentation spaces", () => {
	it("strips IME segmentation spaces when a pure-ASCII pinyin buffer is force-committed", async () => {
		...
		// User typed "kmor" with Chinese pinyin IME active. The IME
		...
		await waitFor(() => {
			expect(editor.textContent).toBe("kmor");
		});
	});
	...
	// strip lands `"kmor"` in the editor AND the caret ends at the end
	// of `"kmor"` (offset 6). Today jsdom clamps the selection so this
	...
	it("keeps the stripped ASCII buffer stable when the next composition happens in Chinese IME", async () => {
		...
		simulateFollowUpComposition(editor, "kmor你好", "你好");

		await waitFor(() => {
			expect(editor.textContent).toBe("kmor你好");
			...
			if (sel && sel.anchorNode === textNode) {
				expect(sel.anchorOffset).toBe("kmor你好".length);
				return;
			}
