 ...
 * tick-by-tick.
 *
 * Output line: `KMOR_PERF_STREAMING_TOTAL=<n>` where `n` =
...
if (typeof window !== "undefined") {
	...
	(
		window as unknown as { __KMOR_DEV_RENDER_STATS__?: unknown }
	).__KMOR_DEV_RENDER_STATS__ = undefined;
...
function readStats(): DevRenderStats | null {
	...
	const stats = (
		window as unknown as { __KMOR_DEV_RENDER_STATS__?: DevRenderStats }
	).__KMOR_DEV_RENDER_STATS__;
	return stats ?? null;
}
...
describe("streaming replay perf", () => {
	it("replays a real Claude SDK capture into the panel and emits KMOR_PERF_STREAMING_TOTAL", async () => {
		resetStats();
		...
		console.log(`KMOR_PERF_STREAMING_TOTAL=${total}`);
		// eslint-disable-next-line no-console
		console.log(`KMOR_PERF_STREAMING_SNAPSHOTS=${snapshotCount}`);
		// eslint-disable-next-line no-console
		console.log(`KMOR_PERF_STREAMING_FIXTURE_ROWS=${countFixtureRows()}`);
