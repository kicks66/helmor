...
# sessions on this repo by setting KMOR_GIT_AUTHOR_NAME and
# KMOR_GIT_AUTHOR_EMAIL as environment variables in THEIR claude.ai/code
# web environment (Settings → Environments → kmor → Environment
...
#
# We deliberately use KMOR_GIT_AUTHOR_* rather than the standard
...
if [ -n "$KMOR_GIT_AUTHOR_NAME" ]; then
	git config user.name "$KMOR_GIT_AUTHOR_NAME" || true
fi
if [ -n "$KMOR_GIT_AUTHOR_EMAIL" ]; then
	git config user.email "$KMOR_GIT_AUTHOR_EMAIL" || true
fi
