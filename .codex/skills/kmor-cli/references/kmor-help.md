# Kmor CLI Top-Level Help

Captured from `kmor --help`.

```text
Remote-control Kmor from the terminal. Works against the same SQLite database the desktop app uses — run commands even while the app is running.
...
Commands:
  ...
  cli-status   Report whether the current Kmor CLI entrypoint is installed to PATH and which data mode it uses
  quit         Ask a running Kmor app to quit (noop when it isn't running)
