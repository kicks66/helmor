...
fn handle_tools_list(request: &Value) -> Value {
    let tools = json!([
        tool_def("kmor_data_info", "Show Kmor data directory, database path, and mode", json!({})),
        tool_def("kmor_repo_list", "List all registered repositories", json!({})),
        tool_def("kmor_repo_add", "Register a local Git repository (creates first workspace automatically)", json!({
        ...
        tool_def("kmor_workspace_list", "List all active workspaces grouped by status", json!({})),
        tool_def("kmor_workspace_show", "Show details for a workspace", json!({
        ...
        tool_def("kmor_workspace_create", "Create a new workspace for a repository", json!({
            "repo": { "type": "string", "description": "Repository UUID or name" }
        }).as_object().map(|o| json!({ "type": "object", "properties": o, "required": ["repo"] })).unwrap()),
        tool_def("kmor_session_list", "List sessions in a workspace", json!({
        ...
        tool_def("kmor_session_create", "Create a new session in a workspace", json!({
            "workspace": { "type": "string", "description": "Workspace UUID or repo-name/directory-name" }
        }).as_object().map(|o| json!({ "type": "object", "properties": o, "required": ["workspace"] })).unwrap()),
        tool_def("kmor_send", "Send a prompt to an AI agent in a workspace", json!({
            "workspace": { "type": "string", "description": "Workspace UUID or repo-name/directory-name" },
