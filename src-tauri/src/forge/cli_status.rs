...
fn bundled_program_token(program: &str) -> Result<String> {
    ...
    bail!("Bundled `{program}` is missing; reinstall Kmor to recover")
}
...
fn github_status_from(status: github_cli::GithubCliStatus) -> Result<ForgeCliStatus> {
    Ok(match status {
        ...
        github_cli::GithubCliStatus::Unavailable { host, message } => ForgeCliStatus::Error {
            ...
            version: None,
            message: format!(
                "Bundled GitHub CLI was not found. Reinstall Kmor to recover. ({message})"
            ),
        },
    ...
    let version = match run_command("glab", ["--version"]) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            ...
            return Ok(ForgeCliStatus::Error {
                ...
                cli_name: "glab".to_string(),
                version: None,
                message: "Bundled GitLab CLI was not found. Reinstall Kmor to recover.".to_string(),
            });
        }
        Err(error) => {
...
mod tests {
    ...
    fn shell_single_quote_handles_embedded_single_quotes() {
        ...
        assert_eq!(
            shell_single_quote("/Apps/Tom's Stuff/Kmor.app/Contents/Resources/vendor/gh/gh"),
            "'/Apps/Tom'\\''s Stuff/Kmor.app/Contents/Resources/vendor/gh/gh'"
        );
