...
pub fn quit() -> Result<()> {
    if service::is_app_running() {
        anyhow::bail!(
            "Kmor is running but the CLI cannot stop it remotely yet. \
             Close the app from the menu bar (Kmor → Quit) or press ⌘Q."
        );
    }
    println!("Kmor is not running.");
    Ok(())
}
