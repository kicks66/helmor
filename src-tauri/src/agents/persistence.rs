...
pub(super) fn persist_turn_message(
    ...
    conn.execute(
        r#"
            ...
            "#,
        params![msg_id, ctx.kmor_session_id, turn.role, content, now],
    )?;
    Ok(msg_id)
}
...
pub(super) fn persist_result_and_finalize(
    ...
    transaction.execute(
        r#"
            ...
            "#,
        params![result_message_id, ctx.kmor_session_id, result_payload, now],
    )?;
...
fn finalize_session_metadata_in_transaction(
    ...
    transaction.execute(
        r#"
            ...
            "#,
        params![ctx.kmor_session_id, ctx.kmor_session_id],
    )?;

    mark_session_read_in_transaction(transaction, &ctx.kmor_session_id)?;
    Ok(())
}
...
mod tests {
    ...
    fn test_exchange_context() -> ExchangeContext {
        ExchangeContext {
            kmor_session_id: "session-1".to_string(),
